...
class A_Adaptive:
    ...
    # Takes the txt files from randGrid and converts it to numpy array
    def parseMaze(self): 
...
##        self.goal_position = [69,44]
    ...
    # Main loop for Repeated A*
        while start.position != goal.position: 
            ...
            opens = [] 
            heapify(opens)
            heappush(opens, start)
            
            close = []

            self.ComputePath(start, goal, opens, close, counter, nodes)
    ...
    # Initialize then go backwards from goal to start
            left_pointer = goal.parent  
            right_pointer = goal
            goal.parent.next = goal

            while left_pointer.position != start.position:
                right_pointer = left_pointer
                left_pointer = right_pointer.parent
                left_pointer.next = right_pointer
                
    # Traverse the path found until we smack into a wall or hit goal, when we smack, change starting position
            while start.position != goal.position:
                for s in range(4):
                    new_y = start.position[0] + yy[s]  
                    new_x = start.position[1] + xx[s]
                    if new_y >= 0 and new_y < self.height and new_x >= 0 and new_x < self.width:
                        if self.maze[new_y][new_x] == 1:
                            self.maze[new_y][new_x] = 5
                if self.maze[right_pointer.position[0]][right_pointer.position[1]] != 5:#####this is traversing down the path found by A* until we get screwed
                    start = right_pointer
    ...
    # Update heuristics of expanded nodes for Adaptive A*
            for i in range(len(close)):
                close[i].h = goal.g - close[i].g

    # When we break out of the loop we have succeeded
        print('poggers ' + str(counter))


    # Performs a Forward A* search       
    def ComputePath(self, start_point, goal_point, opens, close, counter, nodes):

        while opens and goal_point.g > min(opens).f(): ##############
            s = heappop(opens)
            if s.closed == counter: # checks to make sure not in closed list
                continue
            s.closed = counter #insert it into closed list 
            close.append(s)
            expanded_list = self.new_succ(s, counter, nodes, goal_point) #adds new successors to expanded list
    ...
    def new_succ(self, state, counter, nodes, goal): 
            ...
            if new_y >= 0 and new_y < self.height and new_x >= 0 and new_x < self.width: 
                ...
                if self.maze[new_y][new_x] != 5: # checks if the cell is known to be blocked
                    if not nodes[new_y][new_x]: # if the node for the cell does not exist yet, we make it
                        ...
                        expanded_point.h = self.manhattan_distance(expanded_point, goal)
...
if __name__ == '__main__':
    ...
    A_Adaptive_Maze = A_Adaptive(maze, height, width)

    A_Adaptive_Maze.createAgentandTarget()

    start = timeit.default_timer()
        
    A_Adaptive_Maze.A_star()
    
    A_Adaptive_Maze.outputMaze()
