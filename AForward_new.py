...
class Node():
    def __init__(self, y, x, value):
        ...
        self.closed = 0
    def priority(self):
        return ((100000 * (self.g + self.h)) - self.g) # change the -g to +g to test small g tiebreaker method
    def f(self):
            ...
            while start.position != goal.position:
                for s in range(4): 
                    ...
                    if new_y >= 0 and new_y < self.height and new_x >= 0 and new_x < self.width:
                        if self.maze[new_y][new_x] == 1:
                            self.maze[new_y][new_x] = 5
                if self.maze[right_pointer.position[0]][right_pointer.position[1]] != 5:
                    start = right_pointer
                    right_pointer = right_pointer.next
                    if self.maze[start.position[0]][start.position[1]] == 0:
...
if __name__ == '__main__':
    ...
    A_Forward_Maze = A_Forward(maze, height, width)

    A_Forward_Maze.createAgentandTarget()

    start = timeit.default_timer()
    
    A_Forward_Maze.A_star()
    
    A_Forward_Maze.outputMaze()
